...
2. `docker compose config --services` — expect: `web`, `portfolio-api`, `ingest`, `signals`, `db` (no `ib-gateway` / `trading-worker`).
...
- **IBKR**, **trading-worker**, **`/internal/orders`**, **`/mcp/v1/trades`**: enable via `TRADING_ENABLED=true` and optional `docker-compose.coinbase.yml` merge.
